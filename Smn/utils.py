...
# see doc string
def group_elasticities(
    df: pd.DataFrame,
    group_col: list,
    freq: str,
    date_col="date",
    discount_col="discount",
    sales_col="product_quantity",
):
...
def _resample_nlog(
    df, freq="d", date_col="date", discount_col="discount", sales_col="product_quantity"
):
    resample_df = (
        df.resample(freq, on=date_col).agg(
            sales=(sales_col, "sum"),
...
def _elasticity_calc(df, group_col):
    ...
    for i in df[group_col].unique():
        ...
        elasticities_df.loc[len(elasticities_df)] = [
            i,
            model.params.log_discount,
            model.pvalues.log_discount,
            revenue,
            sales,
            mn_price,
            discount,
...
def plot_nformat(df, save_path=None):
    ...
    ax.set_title(
        f"Log Discount vs Log Sales\ny = {intercept:.3f} + {slope:.3f}x (R²={r_value**2:.3f})"
    )
    fig.tight_layout()
...
def bar_plot(df, x_col, y_col, group_name, nlarg_col, nlarg_num=10):
    ...
    # Create the barplot
    sns.barplot(
        data=sorted_data, x=x_col, y=y_col, hue=y_col, edgecolor="black", linewidth=0.5, ax=ax
    )
    ...
    ax.set_title(
        f"{x_col} - Top {nlarg_num} {group_name} by {nlarg_col}",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
    ...
    for i, (v, rev) in enumerate(zip(sorted_data[x_col], sorted_data[nlarg_col])):
        ax.text(v + 0.01 * (1 if v >= 0 else -1), i, f"{v:.2f}", va="center", fontweight="bold")
...
def scatter_plot(df, x_col, y_col, hue_col, group_name, label_col=None, loc="upper right"):
    ...
    g.figure.suptitle(
        f"{y_col.replace('_', ' ')} vs {x_col.replace('_', ' ')} by {group_name.replace('_', ' ')}",
        fontsize=14,
        fontweight="bold",
    )
    ...
    if label_col is not None:
        for idx, row in top_brands.iterrows():
            g.ax.text(row[x_col] + 0.02, row[y_col] + 0.02, row[label_col], fontsize=9, alpha=0.8)
